...
submitStatusBtn.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch("/postTransactionofagent", {
      method: "POST",
...
============================================================ */
document.querySelectorAll("form[action='/postTransactionofagent']").forEach(form => {
  form.addEventListener("submit", async (e) => {
    ...
    const response = await fetch("/postTransactionofagent", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
